...
Cross-view geo-localization asks which satellite tile matches a UAV frame under viewpoint change, GNSS dropout, and weather. University-1652 ([Zheng, Wei, Yang, ACM MM 2020](https://doi.org/10.1145/3394171.3413896)) is the standard drone↔sat protocol; the imagery is multi-gigabyte and gated. This study isolates a prior, CPU-reproducible question on a procedural campus:
...
The design follows the experimental style of WeatherPrompt and MuSe-Net (robustness) and Ctrl-U (variance as an uncertainty instrument), executed as a **simulation**. Reported numbers are not a University-1652 leaderboard.
...
1. Request University-1652 with an academic email ([Request.md](https://github.com/layumi/University1652-Baseline/blob/master/Request.md) → zdzheng12@gmail.com) or gated Hugging Face; follow [`scripts/prepare_u1652.md`](../scripts/prepare_u1652.md). Do not commit the extract.
2. Point `configs/default.yaml` `data.root` at the Baseline `train/`+`test/` tree ([docs/DATASETS.md](../docs/DATASETS.md)).
3. Swap `tiny_cnn` → `resnet18` with ImageNet init (outside CI) and **measure** U1652 R@1 / mAP here. Those cells are **TODO** until then.
4. Re-run the same eight conditions as **eval overlays** on real U1652 drone queries (WeatherPrompt-style), not as a claim that we trained on U1652-WX.
5. Replace MC-Dropout with a proper evidential or Gaussian embedding head if \(\sigma\) remains poorly calibrated.
6. Do not paste LPN/FSRA/WeatherPrompt tables into this file unless those runs are executed here.
